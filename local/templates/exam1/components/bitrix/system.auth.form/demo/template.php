	...
	<ul>
		<li class="att popup-wrap"><a id="hd_singin_but_open" href="" class="btn-toggle"><?=GetMessage("AUTH_LOGIN_LINK_TEXT")?></a>
			<form class="frm-login popup-block" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
				...
				<div class="frm-row">
					<div class="frm-chk">
						<input type="checkbox" id="login" name="USER_REMEMBER" value=<?=$arResult["STORE_PASSWORD"] === "Y" ? "Y" : "N"?>> <label for="login"><?=GetMessage("AUTH_REMEMBER_ME_SHORT")?></label>
					</div>
				</div>
...
<?else:?>
	<nav class="menu-block">
		<ul>
			...
			<li><a href="/?logout=yes&<?=bitrix_sessid_get()?>"><?=Loc::getMessage('AUTH_LOGOUT_BUTTON')?></a>
			</li>
		</ul>
	</nav>
<?endif;?>
